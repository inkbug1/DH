export const Formats: {[k: string]: FormatData} = {
	...
	movexit: {
		...
		name: 'Movexit Clause', 
		desc: 'Moves cut from Pokemon Sword and Shield are not usable.',
		onValidateSet(set) {
			const problems = [];
			if (set.moves) {
	...
	speciesclause: {//I hope I did not break this
		effectType: 'ValidatorRule',
		name: 'Species Clause',
		desc: "Prevents teams from having more than one Pok&eacute;mon from the same species, not counting forms.",
		onBegin() {
